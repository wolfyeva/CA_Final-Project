...
uniform float range_k0 ;
uniform float range_k1;
uniform float range_d0 ;
uniform float range_d1;
uniform float k_s;
uniform float k_b;
uniform float dead_color_factor_1;
uniform float dead_outline_factor_1;
uniform float dead_outline_thickness_1;
uniform vec3 knotColor;// = vec3(0.20,0.20,0.15)
...
void main()
{
    ...
    float dead_color_factor = dead_color_factor_1;
    float dead_outline_factor = 1;
    float dead_outline_thickness = dead_outline_thickness_1;
      ...
      if (T_b[i]<9.0){
        ...
        if (T_b[i]<T_death[i]){ // Dead knot outline
          ...
          if( abs(T_death[i]-t)<dead_outline_thickness){
            dead_outline_factor = dead_outline_factor_1;
          }
        }
      }
    }
    ...
    // Direction LIGHT (Phong lighting model)
